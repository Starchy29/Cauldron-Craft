...
public class MonstersData
{
    ...
    private MonstersData() {
        ...
        monsterTypes[(int)MonsterName.LostSoul] = new MonsterType(MonsterName.LostSoul, new List<Ingredient>() { Ingredient.Decay, Ingredient.Decay, Ingredient.Decay },
            25, 4,
            new Move("Revitalize", 1, MoveType.Heal, Move.Targets.Allies, new RangeSelector(2, false, true), (user, tile) => { LevelGrid.Instance.GetMonster(tile).Heal(6); }, AnimateGlow(2f, new Color(0.2f, 0.7f, 0.9f)), Sounds.SoulHeal, "Heals an ally for 6 health."),
            new StatusMove("Haunt", 2, new StatusAilment(StatusEffect.Haunt, 2, prefabs.spookHaunt), RangeSelector.MeleeSelector, 
                (user, targets) => { AnimationsManager.Instance.QueueAnimation(new ThrustAnimator(user.gameObject, Global.DetermineCenter(targets.Filtered), true)); }, Sounds.Haunt, "The target takes 1.5x damage for 2 turns.")
        );
        ...
        monsterTypes[(int)MonsterName.Demon] = new MonsterType(MonsterName.Demon, new List<Ingredient>() { Ingredient.Decay, Ingredient.Decay, Ingredient.Decay },
            25, 4,
            new Attack("Fireball", 1, 8, new RangeSelector(3, false, true), AnimateProjectile(prefabs.fireball, prefabs.fireballBlast, 10f, Sounds.FireHit), Sounds.FireSpawn, "Deals 8 damage to the target and 4 damage to enemies adjacent to the target.", (user, target, healthLost) => { DealSplashDamage(user, target.Tile, 4); }),
        ...
        monsterTypes[(int)MonsterName.Flytrap] = new MonsterType(MonsterName.Flytrap, new List<Ingredient>() { Ingredient.Flora, Ingredient.Flora, Ingredient.Flora },
            ...
            new Move("Vine Grasp", 2, MoveType.Shift, Move.Targets.Enemies, new DirectionSelector(4, false), PullTarget, null, Sounds.Whoosh, "Pulls the target to the user and slows it for 2 turns.")
        );
        ...
        monsterTypes[(int)MonsterName.Golem] = new MonsterType(MonsterName.Golem, new List<Ingredient>() { Ingredient.Mineral, Ingredient.Mineral, Ingredient.Mineral },
            22, 5,
            new StatusMove("Aura Boost", 1, new StatusAilment(StatusEffect.Power, 1, prefabs.auraStatus), new RangeSelector(2, false, true), AnimateGlow(1.5f, Color.cyan), Sounds.Aura, "Increases an ally's damage by 1.5x for 1 turn."),
        ...
        monsterTypes[(int)MonsterName.Automaton] = new MonsterType(MonsterName.Automaton, new List<Ingredient>() { Ingredient.Mineral, Ingredient.Mineral, Ingredient.Mineral },
            28, 2,
            new Attack("Flame Cannon", 1, 9, new ZoneSelector(5, 2, true), AnimateLobber(prefabs.fireball, 3f, 1f, Sounds.FireHit, prefabs.fireballBlast), Sounds.FireSpawn, "Deals 9 damage in a 2x2 square. Cannot be used if the user has moved this turn."),
        ...
        monsterTypes[(int)MonsterName.Fungus] = new MonsterType(MonsterName.Fungus, new List<Ingredient>() { Ingredient.Flora, Ingredient.Flora, Ingredient.Decay },
            28, 3,
            new StatusMove("Infect", 1, new StatusAilment(StatusEffect.Poison, 3, prefabs.poisonSpores), new RangeSelector(2, false, true), AnimateLobber(prefabs.sporeShot, 0.8f, 0.7f, Sounds.MushroomGrow), Sounds.Whoosh, "Deals 5 damage for 3 turns."),
            new StatusMove("Psychic Spores", 1, new StatusAilment(StatusEffect.Fear, 2, prefabs.fearStatus), RangeSelector.MeleeSelector, meleeAnimation, Sounds.Bump, "Halves the target's damage for 2 turns.")
        );
        ...
        monsterTypes[(int)MonsterName.Jackolantern] = new MonsterType(MonsterName.Jackolantern, new List<Ingredient>() { Ingredient.Flora, Ingredient.Flora, Ingredient.Mineral },
            22, 4,
            new StatusMove("Will o' Wisp", 1, new StatusAilment(StatusEffect.Haunt, 1, prefabs.willOWisps), new RangeSelector(3, false, true), AnimateProjectile(prefabs.wispAttack, null, 2f, Sounds.None, false, false), Sounds.SoulHeal, "The target receives 1.5x damage this turn."),
            new StatusMove("Hex", 2, new StatusAilment(StatusEffect.Cursed, 3, prefabs.hexStatus), new RangeSelector(2, false, true), AnimateGlow(1.5f, new Color(0.5f, 0.1f, 0.8f)), Sounds.Hex, "Makes an ally deal 4 revenge damage for 3 turns.")
        );
        ...
        monsterTypes[(int)MonsterName.Sludge] = new MonsterType(MonsterName.Sludge, new List<Ingredient>() { Ingredient.Decay, Ingredient.Decay, Ingredient.Flora },
            25, 4,
            new Attack("Blob Lob", 1, 8, new RangeSelector(4, false, true), AnimateLobber(prefabs.sludgeLob, 2.0f, 0.6f, Sounds.SludgeHit), Sounds.SludgeToss, "Deals 8 damage."),
            new ZoneMove("Toxic Puddle", 3, ZoneSelector.AdjacentSelector, TileAffector.CreateBlueprint(prefabs.sludgeZone, 3, 0, null, false, false, DealPuddleDamage),
                AnimateStretching(new List<StretchType> { StretchType.Horizontal }, 1.5f, 1f), Sounds.SludgePuddle, "Places a zone that deals 5 damage to enemies that stand on it.")
        );
        ...
        monsterTypes[(int)MonsterName.Fossil] = new MonsterType(MonsterName.Fossil, new List<Ingredient>() { Ingredient.Mineral, Ingredient.Mineral, Ingredient.Decay },
            22, 3,
            new Attack("Bone Shot", 1, 10, new DirectionSelector(5, false), AnimateLinearShot(prefabs.boneShot, 16f, Sounds.Bone), Sounds.Whoosh, "Deals 10 damage."),
            new ZoneMove("Quicksand", 3, new ZoneSelector(2, 2), TileAffector.CreateBlueprint(prefabs.quicksand, 3, 1, null), meleeAnimation, Sounds.Sand, "Places an area that slows enemies passing through.")
        );
        ...
        monsterTypes[(int)MonsterName.Amalgamation] = new MonsterType(MonsterName.Amalgamation, new List<Ingredient>() { Ingredient.Decay, Ingredient.Flora, Ingredient.Mineral },
            ...
            new Move("Mend Flesh", 2, MoveType.Heal, Move.Targets.Allies, SelfSelector.Instance, (user, tile) => user.Heal(4), 
                AnimateStretching(new List<StretchType> { StretchType.Horizontal, StretchType.Vertical, StretchType.Horizontal, StretchType.Vertical  }, 0.5f, 0.3f), Sounds.StretchFlesh, "Heal 4 health")
        );
    }
    ...
    private static Move.AnimationFunction AnimateProjectile(GameObject projectilePrefab, GameObject destroyParticlePrefab, float speed, Sounds hitSound, bool reversed = false, bool rotate = true) {
        return (Monster user, Selection targets) => {
            ...
            if(hitSound != Sounds.None) {
                AnimationsManager.Instance.QueueSound(hitSound);
            }
        };
    }

    private static Move.AnimationFunction AnimateLobber(GameObject prefab, float height, float duration, Sounds hitSound, GameObject destroyParticle = null) {
        return (Monster user, Selection targets) => {
            LevelGrid level = LevelGrid.Instance;
            Vector3 start = level.Tiles.GetCellCenterWorld((Vector3Int)user.Tile);
    ...
    private static Move.AnimationFunction AnimateLinearShot(GameObject projectilePrefab, float speed, Sounds hitSound) {
        return (Monster user, Selection targets) => {
            LevelGrid level = LevelGrid.Instance;
            Vector3 start = level.Tiles.GetCellCenterWorld((Vector3Int)user.Tile);
    ...
    private static void AnimateBarbBullet(Monster user, Selection targets) {
        AnimateLinearShot(PrefabContainer.Instance.thornShot, 30f, Sounds.None)(user, targets);
    ...
    private static void DashSlash(Monster user, Vector2Int target) {
        ...
        AnimationsManager.Instance.QueueSound(Sounds.Whoosh);
